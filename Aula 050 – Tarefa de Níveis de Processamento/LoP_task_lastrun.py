...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='C:\\Users\\limap\\OneDrive\\Área de Trabalho\\Python para Psicólogos\\psychopy-em-portugues\\Aula 050 – Tarefa de Níveis de Processamento\\LoP_task_lastrun.py',
...
def run(expInfo, thisExp, win, inputs, globalClock=None, thisSession=None):
    ...
    # lista de instruções
    instruction_list = ["instruction1.jpg", "instruction2.jpg", "instruction3.jpg", f"instruction4_{my_condition}.jpg", "instruction5.jpg"]
    ...
    words = [
             ["bola", "cola", "roupa", "Ele pegou a ____ para jogar futebol", "Ele pegou o ferro para passar a ____"],
             ...
             ["natação", "eleição", "maratona", "A ____ é um esporte praticado em uma piscina", "A ____ é uma corrida de 42 km"],
             ["baleia", "sereia", "girafa", "Na história, Pinóquio foi engolido por uma ____", "A ____ tem um pescoço longo"],
             ["chupeta", "camiseta", "bicicleta", "O bebê chorava porque derrubou sua ____", "Ao invés de ir à pé, ele foi de ____"]
             ]
    ...
    prompt_msg = visual.TextStim(win=win, name='prompt_msg',
        text='',
        font='Times New Roman',
        units='norm', pos=(0, 0.65), height=0.15, wrapWidth=1.8, ori=0.0, 
    ...
    button_img = visual.ImageStim(
        win=win,
        name='button_img', units='norm', 
        image='buttons.jpg', mask=None, anchor='center',
        ori=0.0, pos=(0, -0.6), size=(.5, .5),
    ...
    for thisMain_task_trial in main_task_trials:
        ...
        if my_condition == "structural":
            ...
            if trials[main_task_trials.thisN] == 1:
                word = words[main_task_trials.thisN][0].upper()
                corr = 1
            elif trials[main_task_trials.thisN] == 0:
        ...
        # keep track of which components have finished
        main_taskComponents = [prompt_msg, word_msg, main_task_resp, button_img]
        ...
        while continueRoutine:
            ...
            if prompt_msg.status == NOT_STARTED and tThisFlip >= 0-frameTolerance:
                # keep track of start time/frame for later
            ...
            if word_msg.status == NOT_STARTED and tThisFlip >= 0.5-frameTolerance:
                # keep track of start time/frame for later
            ...
            if main_task_resp.status == NOT_STARTED and tThisFlip >= 0.5-frameTolerance:
                # keep track of start time/frame for later
            ...
            # if button_img is starting this frame...
            if button_img.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
                # keep track of start time/frame for later
                button_img.frameNStart = frameN  # exact frame index
                button_img.tStart = t  # local t and not account for scr refresh
                button_img.tStartRefresh = tThisFlipGlobal  # on global time
                win.timeOnFlip(button_img, 'tStartRefresh')  # time at next scr refresh
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'button_img.started')
                # update status
                button_img.status = STARTED
                button_img.setAutoDraw(True)
            
            # if button_img is active this frame...
            if button_img.status == STARTED:
                # update params
                pass
