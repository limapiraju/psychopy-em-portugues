...
expName = 'aula_046a'  # from the Builder filename that created this script
expInfo = {
...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='C:\\Users\\limap\\OneDrive\\Área de Trabalho\\Aula 046\\aula_046a_lastrun.py',
    ...
    for thisStudy_period in study_period:
        ...
        while continueRoutine and routineTimer.getTime() < 0.2:
            ...
            if cue_study.status == STARTED:
                ...
                if tThisFlipGlobal > cue_study.tStartRefresh + 0.2-frameTolerance:
            ...
            if target_study.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > target_study.tStartRefresh + 0.2-frameTolerance:
                    # keep track of stop time/frame for later
    ...
    for thisTest_period in test_period:
        ...
        while continueRoutine and routineTimer.getTime() < 7.0:
            ...
            if len(target_test.text) > max_length:
                target_teste.text.strip()[:max_length]
