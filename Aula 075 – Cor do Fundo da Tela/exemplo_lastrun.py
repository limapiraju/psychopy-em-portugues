...
bg_color = [-0.9922, -0.7020, 0.5137] # azul
...
expName = 'exemplo'  # from the Builder filename that created this script
...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='C:\\Users\\limap\\OneDrive\\Área de Trabalho\\Aula 075 – Cor do Fundo da Tela\\exemplo_lastrun.py',
...
def setupWindow(expInfo=None, win=None):
    ...
    if win is None:
        ...
        win = visual.Window(
            ...
            monitor='testMonitor', color=bg_color, colorSpace='rgb',
...
def setupDevices(expInfo, thisExp, win):
    ...
    if deviceManager.getDevice('testing_resp') is None:
        # initialise testing_resp
        testing_resp = deviceManager.addDevice(
            deviceClass='keyboard',
            deviceName='testing_resp',
        )
...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    # se não temos um arquivo de IDs, crie um
    if not exists("unique_IDs.txt"):
        ...
        arquivo.close()
    # por outro lado, se existir, apenas abra esse arquivo para modificarmos ele
    else:
    ...
    thanks_txt = f"""Tarefa concluída, {first_name}.
    ...
    instr_msg = visual.TextStim(win=win, name='instr_msg',
        text='',
        font='Times New Roman',
        units='norm', pos=(0, 0.2), height=0.12, wrapWidth=1.8, ori=0, 
    ...
    images = {"mamífero": ["gato", "vaca", "cachorro", "macaco", "porco"],
              "ave": ["galinha", "pombo", "águia", "coruja", "papagaio"]}
            
    stimuli = []
    
    for key, value in images.items():
        for v in value:
            stimuli.append((key, v))
    
    # stimuli = [("mamífero", "gato"), ("mamífero", "vaca"), ..., ("ave", "papagaio")]
            
    stimuli = stimuli[:] * 1 # mudar o valor caso queira mais repetições de cada imagem
    random.shuffle(stimuli)
    
    # contrabalanceamento da designação das alternativas aos botões de resposta
    
    # Mamífero esquerda, ave direita
    if random.random() > 0.5:
        mammal_resp, bird_resp = "left", "right"
        left_text, right_text = "Mamífero", "Ave"
        instr_text_left, instr_text_right = "um mamífero", "uma ave"
        # print("Mamífero esquerda, ave direita")
    # Mamífero direita, ave esquerda
    else:
        mammal_resp, bird_resp = "right", "left"
        left_text, right_text = "Ave", "Mamífero"
        instr_text_left, instr_text_right = "uma ave", "um mamífero"
        # print("Ave esquerda, mamífero direita")
    
    instruction_list = [# instruções gerais
    f"""
    Em seguida, você verá imagens de animais.
    
    Sua tarefa é categorizar cada animal como sendo {instr_text_left} ou {instr_text_right}.
    """,
    f"""
    Quando o animal for {instr_text_left}, pressione a tecla [←] o mais rápido possível.
    
    Quando o animal for {instr_text_right}, pressione a tecla [→] o mais rápido possível.""",
    f"""
    Seu tempo para responder será limitado. Por isso, responda o mais rápido que puder. E lembre-se: 
    
        - {left_text}: [←]
        - {right_text}: [→]
    """,
    """
    Em seguida, daremos início à tarefa. Favor chamar o pesquisador se tiver alguma pergunta.
    ...
    image = visual.ImageStim(
        ...
        ori=0.0, pos=(0, 0.2), size=(0.7, 0.7),
        color=[1,1,1], colorSpace='rgb', opacity=None,
        flipHoriz=False, flipVert=False,
        texRes=128.0, interpolate=True, depth=-1.0)
    testing_resp = keyboard.Keyboard(deviceName='testing_resp')
    testing_left_button = visual.ImageStim(
        win=win,
        name='testing_left_button', units='norm', 
        image='images/left_button.jpg', mask=None, anchor='center',
        ori=0.0, pos=(-0.2, -0.6), size=(0.25, 0.3),
        color=[1,1,1], colorSpace='rgb', opacity=None,
        flipHoriz=False, flipVert=False,
        texRes=128.0, interpolate=True, depth=-3.0)
    testing_right_button = visual.ImageStim(
        win=win,
        name='testing_right_button', units='norm', 
        image='images/right_button.jpg', mask=None, anchor='center',
        ori=0.0, pos=(0.2, -0.6), size=(0.25, 0.3),
        color=[1,1,1], colorSpace='rgb', opacity=None,
        flipHoriz=False, flipVert=False,
        texRes=128.0, interpolate=True, depth=-4.0)
    testing_left_msg = visual.TextStim(win=win, name='testing_left_msg',
        text='',
        font='Times New Roman',
        units='norm', pos=(-0.2, -0.85), height=0.1, wrapWidth=1.8, ori=0.0, 
        color='white', colorSpace='rgb', opacity=None, 
        languageStyle='LTR',
        depth=-5.0);
    testing_right_msg = visual.TextStim(win=win, name='testing_right_msg',
        text='',
        font='Times New Roman',
        units='norm', pos=(0.2, -0.85), height=0.1, wrapWidth=1.8, ori=0.0, 
        color='white', colorSpace='rgb', opacity=None, 
    ...
    thisExp.addData('ID', participant_code)
    ...
    for thisInstruction in instructions:
        ...
        # Se a instrução atual é igual ao comprimento da lista de instruções
            ...
            instructions.finished = True # encerra o loop das instruções de teste
    ...
    # set up handler to look after randomisation of conditions etc
    trials = data.TrialHandler(nReps=len(stimuli), method='sequential', 
        extraInfo=expInfo, originPath=-1,
        trialList=[None],
    ...
    for thisTrial in trials:
        ...
        stimulus = f"images/{stimuli[trials.thisN][1]}.png"
        
        # se a tentativa é um mamífero
        if stimuli[trials.thisN][0] == "mamífero":
            if mammal_resp == "left":
                corr_resp = "left" # sorteou left para mamíferos
            else:
                corr_resp = "right" # sorteou right para mamíferos
        # se a tentativa é uma ave
        elif stimuli[trials.thisN][0] == "ave":
            if bird_resp == "left":
                corr_resp = "left" # sorteou left para aves
            else:
                corr_resp = "right" # sorteou right para aves
        
        image.setImage(stimulus)
        # create starting attributes for testing_resp
        testing_resp.keys = []
        testing_resp.rt = []
        _testing_resp_allKeys = []
        testing_left_msg.setText(left_text)
        testing_right_msg.setText(right_text)
        # keep track of which components have finished
        trialComponents = [image, testing_resp, testing_left_button, testing_right_button, testing_left_msg, testing_right_msg]
        ...
        while continueRoutine and routineTimer.getTime() < 2.5:
            ...
            # *testing_resp* updates
            waitOnFlip = False
            
            # if testing_resp is starting this frame...
            if testing_resp.status == NOT_STARTED and tThisFlip >= 0.5-frameTolerance:
                # keep track of start time/frame for later
                testing_resp.frameNStart = frameN  # exact frame index
                testing_resp.tStart = t  # local t and not account for scr refresh
                testing_resp.tStartRefresh = tThisFlipGlobal  # on global time
                win.timeOnFlip(testing_resp, 'tStartRefresh')  # time at next scr refresh
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'testing_resp.started')
                # update status
                testing_resp.status = STARTED
                # keyboard checking is just starting
                waitOnFlip = True
                win.callOnFlip(testing_resp.clock.reset)  # t=0 on next screen flip
                win.callOnFlip(testing_resp.clearEvents, eventType='keyboard')  # clear events on next screen flip
            
            # if testing_resp is stopping this frame...
            if testing_resp.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > testing_resp.tStartRefresh + 2-frameTolerance:
                    # keep track of stop time/frame for later
                    testing_resp.tStop = t  # not accounting for scr refresh
                    testing_resp.tStopRefresh = tThisFlipGlobal  # on global time
                    testing_resp.frameNStop = frameN  # exact frame index
                    # add timestamp to datafile
                    thisExp.timestampOnFlip(win, 'testing_resp.stopped')
                    # update status
                    testing_resp.status = FINISHED
                    testing_resp.status = FINISHED
            if testing_resp.status == STARTED and not waitOnFlip:
                theseKeys = testing_resp.getKeys(keyList=['left', 'right'], ignoreKeys=["escape"], waitRelease=False)
                _testing_resp_allKeys.extend(theseKeys)
                if len(_testing_resp_allKeys):
                    testing_resp.keys = _testing_resp_allKeys[0].name  # just the first key pressed
                    testing_resp.rt = _testing_resp_allKeys[0].rt
                    testing_resp.duration = _testing_resp_allKeys[0].duration
                    # was this correct?
                    if (testing_resp.keys == str(corr_resp)) or (testing_resp.keys == corr_resp):
                        testing_resp.corr = 1
                    else:
                        testing_resp.corr = 0
            
            # *testing_left_button* updates
            
            # if testing_left_button is starting this frame...
            if testing_left_button.status == NOT_STARTED and tThisFlip >= 0-frameTolerance:
                # keep track of start time/frame for later
                testing_left_button.frameNStart = frameN  # exact frame index
                testing_left_button.tStart = t  # local t and not account for scr refresh
                testing_left_button.tStartRefresh = tThisFlipGlobal  # on global time
                win.timeOnFlip(testing_left_button, 'tStartRefresh')  # time at next scr refresh
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'testing_left_button.started')
                # update status
                testing_left_button.status = STARTED
                testing_left_button.setAutoDraw(True)
            
            # if testing_left_button is active this frame...
            if testing_left_button.status == STARTED:
                # update params
                pass
            
            # if testing_left_button is stopping this frame...
            if testing_left_button.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > testing_left_button.tStartRefresh + 2.5-frameTolerance:
                    # keep track of stop time/frame for later
                    testing_left_button.tStop = t  # not accounting for scr refresh
                    testing_left_button.tStopRefresh = tThisFlipGlobal  # on global time
                    testing_left_button.frameNStop = frameN  # exact frame index
                    # add timestamp to datafile
                    thisExp.timestampOnFlip(win, 'testing_left_button.stopped')
                    # update status
                    testing_left_button.status = FINISHED
                    testing_left_button.setAutoDraw(False)
            
            # *testing_right_button* updates
            
            # if testing_right_button is starting this frame...
            if testing_right_button.status == NOT_STARTED and tThisFlip >= 0-frameTolerance:
                # keep track of start time/frame for later
                testing_right_button.frameNStart = frameN  # exact frame index
                testing_right_button.tStart = t  # local t and not account for scr refresh
                testing_right_button.tStartRefresh = tThisFlipGlobal  # on global time
                win.timeOnFlip(testing_right_button, 'tStartRefresh')  # time at next scr refresh
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'testing_right_button.started')
                # update status
                testing_right_button.status = STARTED
                testing_right_button.setAutoDraw(True)
            
            # if testing_right_button is active this frame...
            if testing_right_button.status == STARTED:
                # update params
                pass
            
            # if testing_right_button is stopping this frame...
            if testing_right_button.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > testing_right_button.tStartRefresh + 2.5-frameTolerance:
                    # keep track of stop time/frame for later
                    testing_right_button.tStop = t  # not accounting for scr refresh
                    testing_right_button.tStopRefresh = tThisFlipGlobal  # on global time
                    testing_right_button.frameNStop = frameN  # exact frame index
                    # add timestamp to datafile
                    thisExp.timestampOnFlip(win, 'testing_right_button.stopped')
                    # update status
                    testing_right_button.status = FINISHED
                    testing_right_button.setAutoDraw(False)
            
            # *testing_left_msg* updates
            
            # if testing_left_msg is starting this frame...
            if testing_left_msg.status == NOT_STARTED and tThisFlip >= 0-frameTolerance:
                # keep track of start time/frame for later
                testing_left_msg.frameNStart = frameN  # exact frame index
                testing_left_msg.tStart = t  # local t and not account for scr refresh
                testing_left_msg.tStartRefresh = tThisFlipGlobal  # on global time
                win.timeOnFlip(testing_left_msg, 'tStartRefresh')  # time at next scr refresh
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'testing_left_msg.started')
                # update status
                testing_left_msg.status = STARTED
                testing_left_msg.setAutoDraw(True)
            
            # if testing_left_msg is active this frame...
            if testing_left_msg.status == STARTED:
                # update params
                pass
            
            # if testing_left_msg is stopping this frame...
            if testing_left_msg.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > testing_left_msg.tStartRefresh + 2.5-frameTolerance:
                    # keep track of stop time/frame for later
                    testing_left_msg.tStop = t  # not accounting for scr refresh
                    testing_left_msg.tStopRefresh = tThisFlipGlobal  # on global time
                    testing_left_msg.frameNStop = frameN  # exact frame index
                    # add timestamp to datafile
                    thisExp.timestampOnFlip(win, 'testing_left_msg.stopped')
                    # update status
                    testing_left_msg.status = FINISHED
                    testing_left_msg.setAutoDraw(False)
            
            # *testing_right_msg* updates
            
            # if testing_right_msg is starting this frame...
            if testing_right_msg.status == NOT_STARTED and tThisFlip >= 0-frameTolerance:
                # keep track of start time/frame for later
                testing_right_msg.frameNStart = frameN  # exact frame index
                testing_right_msg.tStart = t  # local t and not account for scr refresh
                testing_right_msg.tStartRefresh = tThisFlipGlobal  # on global time
                win.timeOnFlip(testing_right_msg, 'tStartRefresh')  # time at next scr refresh
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'testing_right_msg.started')
                # update status
                testing_right_msg.status = STARTED
                testing_right_msg.setAutoDraw(True)
            
            # if testing_right_msg is active this frame...
            if testing_right_msg.status == STARTED:
                # update params
                pass
            
            # if testing_right_msg is stopping this frame...
            if testing_right_msg.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > testing_right_msg.tStartRefresh + 2.5-frameTolerance:
                    # keep track of stop time/frame for later
                    testing_right_msg.tStop = t  # not accounting for scr refresh
                    testing_right_msg.tStopRefresh = tThisFlipGlobal  # on global time
                    testing_right_msg.frameNStop = frameN  # exact frame index
                    # add timestamp to datafile
                    thisExp.timestampOnFlip(win, 'testing_right_msg.stopped')
                    # update status
                    testing_right_msg.status = FINISHED
                    testing_right_msg.setAutoDraw(False)
        ...
        thisExp.addData('ID', participant_code)
        thisExp.addData("category", stimuli[trials.thisN][0])
        thisExp.addData("exemplar", stimuli[trials.thisN][1])
        thisExp.addData('corr_resp', corr_resp)
        # check responses
        if testing_resp.keys in ['', [], None]:  # No response was made
            testing_resp.keys = None
            # was no response the correct answer?!
            if str(corr_resp).lower() == 'none':
               testing_resp.corr = 1;  # correct non-response
            else:
               testing_resp.corr = 0;  # failed to respond (incorrectly)
        # store data for trials (TrialHandler)
        trials.addData('testing_resp.keys',testing_resp.keys)
        trials.addData('testing_resp.corr', testing_resp.corr)
        if testing_resp.keys != None:  # we had a response
            trials.addData('testing_resp.rt', testing_resp.rt)
            trials.addData('testing_resp.duration', testing_resp.duration)
    ...
    # completed len(stimuli) repeats of 'trials'
