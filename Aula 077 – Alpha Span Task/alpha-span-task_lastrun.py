...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='C:\\Users\\limap\\OneDrive\\Área de Trabalho\\Aula 077 – Alpha Span Task\\alpha-span-task_lastrun.py',
...
for thisAlpha_span_task_trial in alpha_span_task_trials:
    ...
    # se é a 1ª tentativa do par (índice par: 0, 2, 4…)
    if (alpha_span_task_trials.thisN % 2) == 0:
        # reseta o sucesso
        success_flag = False
    ...
    # só considera tentativa correta se a resposta tiver o mesmo comprimento
    # que a lista alvo
    if len(participant_list) == len(correct_list):
        aux = 0
        for item, target in zip(participant_list, correct_list):   
            if item.lower() == target.lower():
                aux += 1
    else:
        aux = 0  # marcador de resposta com comprimento incorreto
