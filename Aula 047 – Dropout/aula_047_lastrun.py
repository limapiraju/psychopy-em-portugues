...
expName = 'aula_047'  # from the Builder filename that created this script
expInfo = {
...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='C:\\Users\\limap\\OneDrive\\Área de Trabalho\\Python para Psicólogos\\psychopy-em-portugues\\Aula 047\\aula_047_lastrun.py',
...
total_itens = 40 # mudar para número de itens
max_cycles = 4 # número máximo de ciclos no experimento

print(f"Estudo: {study_drop}; Teste: {test_drop}")
...
back_button = visual.ButtonStim(win, 
    ...
    fillColor=[-1.0000, -1.0000, 0.0902], borderColor=[1.0000, 1.0000, 1.0000],
...
# conta número de pares, ignorando o cabeçalho do arquivo
numero_de_itens = len(arquivo[1:]) 
...
for thisInstruction in instructions:
    ...
    instruction_image.setOpacity(opa[current_instruction])
...
# set up handler to look after randomisation of conditions etc
cycle = data.TrialHandler(nReps=max_cycles, method='random', 
    extraInfo=expInfo, originPath=-1,
    trialList=[None],
...
for thisCycle in cycle:
    ...
    for thisStudy_period in study_period:
        ...
        # checa se número de itens é zero na condição Sn
        if (study_drop == True) and numero_de_itens == 0:
            continueRoutine = False
            if (test_drop == True): # SnTn
                cycle.finished = True # encerre os ciclos
            else:
                study_period.finished = True # encerre só o período de estudo
        ...
        while continueRoutine and routineTimer.getTime() < 5.0:
            ...
            if cue_study.status == STARTED:
                ...
                if tThisFlipGlobal > cue_study.tStartRefresh + 5-frameTolerance:
            ...
            if target_study.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > target_study.tStartRefresh + 5-frameTolerance:
                    # keep track of stop time/frame for later
    ...
    for thisDistractor_period in distractor_period:
        ...
        # keep track of which components have finished
        distractorComponents = [math_problem, math_resp, math_line, math_prompt]
    ...
    for thisTest_period in test_period:
        ...
        if (test_period.thisN == 0) and (cycle.thisN < max_cycles):
            path_next_cycle = f"{expInfo['participant']}/cycle_{cycle.thisN + 2}.csv"
            if not exists(path_next_cycle):
                novo_arquivo = open(path_next_cycle, "w")
                # criar cabeçalho do novo arquivo
                novo_arquivo.write("English,Portuguese\n")
                
                # para debuggar
                print(f"{path_next_cycle} criado com sucesso!")
                
                # mede o comprimento do arquivo do atul loop de ciclos
                path_current_cycle = f"{expInfo['participant']}/cycle_{cycle.thisN + 1}.csv"
                arquivo = open(path_current_cycle, "r").readlines()
                # se a condição é Tn
                if test_drop:
                    # com dropout (Tn)
                    numero_de_itens = len(arquivo[1:]) # conta número de pares
                else:
                    # sem dropout (T)
                    numero_de_itens = total_itens
                
                # para debuggar
                print(f"Ciclo {cycle.thisN + 1}: {numero_de_itens}")      
        
        # checa se número de itens é zero na condição Tn
        if (test_drop == True) and numero_de_itens == 0:
            continueRoutine = False
            if (study_drop == True): # SnTn
                cycle.finished = True # encerre os ciclos
            else:
                test_period.finished = True # encerre só o período de estudo  
        ...
        while continueRoutine and routineTimer.getTime() < 8.0:
            ...
            if len(target_test.text) > max_length:
                target_test.text.strip()[:max_length]
            ...
            if cue_test.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > cue_test.tStartRefresh + 8-frameTolerance:
                    # keep track of stop time/frame for later
            ...
            if target_test.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > target_test.tStartRefresh + 8-frameTolerance:
                    # keep track of stop time/frame for later
        ...
        if (study_drop == True) and (test_drop == True):
            condition = "SnTn"
        elif (study_drop == True) and (test_drop == False):
            condition = "SnT"
        elif (study_drop == False) and (test_drop == True):
            condition = "STn"
        elif (study_drop == False) and (test_drop == False):
            condition = "ST"
        ...
        if (response_corr == 0) and (auxiliar in arquivo):
            novo_arquivo.write(auxiliar)
            # para debuggar
            print(f"Salvei o par {English}–{Portuguese}!")
            
        # se estamos na última iteração do loop test_period
        if (test_period.thisN == (numero_de_itens - 1)):
            novo_arquivo.close()
            # checa se há dropout em períodos de estudo
            if study_drop == True: # Sn
                study_file = path_next_cycle
            # checa se há dropout em períodos de teste
            if test_drop == True: # Tn
                test_file = path_next_cycle
...
# completed max_cycles repeats of 'cycle'
