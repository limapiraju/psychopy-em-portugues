...
expName = 'aula043_solucao1'  # from the Builder filename that created this script
expInfo = {
    'participante': '',
}
...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='C:\\Users\\limap\\OneDrive\\Área de Trabalho\\Aula 043\\aula 043, solução 1_lastrun.py',
...
study_phase = data.TrialHandler(nReps=1, method='random', 
    ...
    trialList=data.importConditions('palavras.xlsx', selection='0:14'),
...
for thisTest_phase in test_phase:
    ...
    for thisComponent in recognitionComponents:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    # Run 'End Routine' code from recog_code
    if recog_resp.keys == "left":
        r_k_routine = 1
    else:
        r_k_routine = 0
    # check responses
    ...
    # set up handler to look after randomisation of conditions etc
    remember_know_trial = data.TrialHandler(nReps=r_k_routine, method='random', 
        extraInfo=expInfo, originPath=-1,
        trialList=[None],
        seed=None, name='remember_know_trial')
    thisExp.addLoop(remember_know_trial)  # add the loop to the experiment
    thisRemember_know_trial = remember_know_trial.trialList[0]  # so we can initialise stimuli with some values
    # abbreviate parameter names if possible (e.g. rgb = thisRemember_know_trial.rgb)
    if thisRemember_know_trial != None:
        for paramName in thisRemember_know_trial:
            exec('{} = thisRemember_know_trial[paramName]'.format(paramName))
    
    for thisRemember_know_trial in remember_know_trial:
        currentLoop = remember_know_trial
        # abbreviate parameter names if possible (e.g. rgb = thisRemember_know_trial.rgb)
        if thisRemember_know_trial != None:
            for paramName in thisRemember_know_trial:
                exec('{} = thisRemember_know_trial[paramName]'.format(paramName))
        ...
        if r_k_resp.keys in ['', [], None]:  # No response was made
            r_k_resp.keys = None
        remember_know_trial.addData('r_k_resp.keys',r_k_resp.keys)
        if r_k_resp.keys != None:  # we had a response
            remember_know_trial.addData('r_k_resp.rt', r_k_resp.rt)
        # the Routine "remember_know" was not non-slip safe, so reset the non-slip timer
        routineTimer.reset()
        thisExp.nextEntry()
        
    # completed r_k_routine repeats of 'remember_know_trial'
    
    # get names of stimulus parameters
    if remember_know_trial.trialList in ([], [None], None):
        params = []
    else:
        params = remember_know_trial.trialList[0].keys()
    # save data for this loop
    remember_know_trial.saveAsExcel(filename + '.xlsx', sheetName='remember_know_trial',
        stimOut=params,
        dataOut=['n','all_mean','all_std', 'all_raw'])
    remember_know_trial.saveAsText(filename + 'remember_know_trial.csv', delim=',',
        stimOut=params,
        dataOut=['n','all_mean','all_std', 'all_raw'])
    thisExp.nextEntry()
