...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='C:\\Users\\limap\\OneDrive\\Área de Trabalho\\Aula 073 – Keep Track Task, Parte III\\ktt_lastrun.py',
...
prompt_recall = visual.TextStim(win=win, name='prompt_recall',
    ...
    languageStyle='LTR',
    depth=-2.0);
multiple_choice_options = visual.TextStim(win=win, name='multiple_choice_options',
    text='',
    font='Times New Roman',
...
# set up handler to look after randomisation of conditions etc
keeping_track_trials = data.TrialHandler(nReps=N_TRIALS, method='sequential', 
    extraInfo=expInfo, originPath=-1,
    trialList=[None],
...
for thisKeeping_track_trial in keeping_track_trials:
    ...
    for thisKeeping_track_word in keeping_track_word:
        ...
        # keep track of which components have finished
        word_trialComponents = [to_be_remembered_categories, to_be_remembered]
        ...
        for thisComponent in word_trialComponents:
            if hasattr(thisComponent, "setAutoDraw"):
                thisComponent.setAutoDraw(False)
        # Run 'End Routine' code from word_trial_code
        thisExp.addData('participant_code', participant_code)
    ...
    for thisKeeping_track_question in keeping_track_questions:
        ...
        multiple_choice_options.alignText = "left"
        ...
        multiple_choice_options.setText(options)
        recall_response.keys = []
        recall_response.rt = []
        _recall_response_allKeys = []
        # keep track of which components have finished
        recallComponents = [to_be_remembered_categories_2, prompt_recall, multiple_choice_options, recall_response]
        ...
        while continueRoutine:
            ...
            # *multiple_choice_options* updates
            if multiple_choice_options.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
                # keep track of start time/frame for later
                multiple_choice_options.frameNStart = frameN  # exact frame index
                multiple_choice_options.tStart = t  # local t and not account for scr refresh
                multiple_choice_options.tStartRefresh = tThisFlipGlobal  # on global time
                win.timeOnFlip(multiple_choice_options, 'tStartRefresh')  # time at next scr refresh
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'multiple_choice_options.started')
                multiple_choice_options.setAutoDraw(True)
        ...
        thisExp.addData('correct_resp', trials[keeping_track_trials.thisN]['questions'][keeping_track_questions.thisN]['correct_index'])
        thisExp.addData('participant_resp', recall_response.keys[-1])
...
# completed N_TRIALS repeats of 'keeping_track_trials'
