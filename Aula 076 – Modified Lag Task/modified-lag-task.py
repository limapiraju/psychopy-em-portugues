...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='C:\\Users\\limap\\OneDrive\\Área de Trabalho\\Aula 074 – Modified Lag Task\\modified-lag-task.py',
...
""",
"""
Por exemplo, suponha que a sequência apresentada foi: Argentina, Brasil, Chile, Dinamarca e que a pergunta seja "Qual foi a penúltima palavra da lista?".

Neste caso, a resposta correta será Chile, pois essa foi a penúltima palavra da lista anterior.
""",
"""
Por exemplo, suponha que a sequência apresentada foi: Argentina, Brasil, Chile, Dinamarca e que a pergunta seja "Qual foi a antepenúltima palavra da lista?".

Neste caso, a resposta correta será Brasil, pois essa foi a antepenúltima palavra da lista anterior.
""",
"""
Por exemplo, suponha que a sequência apresentada foi: Argentina, Brasil, Chile, Dinamarca e que a pergunta seja "Qual foi a quarta palavra, a contar do final da lista?".
...
for thisModified_lag_task_trial in modified_lag_task_trials:
    ...
    thisExp.addData('participant_code', participant_code)
    thisExp.addData('word', my_word)
    thisExp.addData('list_len', list_len)
    thisExp.addData('n_back', n_back)
    thisExp.addData('target_index_0b', target_index_0b)
    thisExp.addData('target_index_1b', target_index_1b)
    thisExp.addData('list', my_list)
    thisExp.addData('target_word', target_word)
    thisExp.addData('participant_response', target_resp.text.strip())
    
    if target_resp.text.strip() == target_word:
        score = 1
        weighted_score = list_len + 1
    else:
        score = 0
        weighted_score = 0
        
    thisExp.addData('score', score)  
    thisExp.addData('weighted_score', weighted_score)
