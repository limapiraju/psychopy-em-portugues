...
expName = 'clock v. 4'  # from the Builder filename that created this script
...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='D:\\Python_files\\PsychoPy\\YouTube\\Aulas\\Aula 030 – Relógios\\exemplo 4\\clock v. 4_lastrun.py',
...
for thisTrial in trials:
    ...
    relogio = core.CountdownTimer(start = 66)
    
    mins = math.floor(relogio.getTime() / 60)
    segs = math.floor(relogio.getTime() - mins * 60)
    trial_text.setText("Tentativa " + str((trials.thisN + 1)))
    key_resp.keys = []
    key_resp.rt = []
    _key_resp_allKeys = []
    # keep track of which components have finished
    trialComponents = [trial_text, text, key_resp]
    ...
    while continueRoutine:
        ...
        if int(relogio.getTime()) <= 45:
            color = "red"
        
        if int(relogio.getTime()) == 0:
        ...
        if text.status == STARTED:  # only update if drawing
            ...
            text.setText(str(int(mins)).zfill(2) + ':' + str(int(segs)).zfill(2), log=False)
