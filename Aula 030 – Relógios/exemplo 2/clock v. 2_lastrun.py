...
expName = 'clock v. 2'  # from the Builder filename that created this script
...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='D:\\Python_files\\PsychoPy\\YouTube\\Aulas\\Aula 030 – Relógios\\exemplo 2\\clock v. 2_lastrun.py',
...
for thisTrial in trials:
    ...
    while continueRoutine:
        ...
        if int(relogio.getTime()) == 5:
            # relogio.add(10)
            relogio.reset(newT = 0.0)
        ...
        if text.status == STARTED:  # only update if drawing
            text.setText(f"{relogio.getTime():.3f}", log=False)
