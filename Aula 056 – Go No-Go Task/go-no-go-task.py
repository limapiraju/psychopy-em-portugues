...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='C:\\Users\\limap\\OneDrive\\Área de Trabalho\\Aula 056 – Go No-Go Task\\go-no-go-task.py',
...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    N_reps = [4, 4, 20]        # número de repetições por tarefa
    ...
    Sua tarefa será pressionar [BARRA DE ESPAÇO] sempre que círculo tiver a cor VERDE. Quando o círculo tiver a cor VERMELHA, você não deverá responder. 
    ...
    Sua tarefa será pressionar [BARRA DE ESPAÇO] sempre que o círculo for verde OU tiver o símbolo ✓ dentro dele. Quando o círculo for vermelho E o símbolo for ✗, você não deverá responder. 
    ...
    for thisTask in tasks:
        ...
        for thisTrial in trials:
            ...
            while continueRoutine and routineTimer.getTime() < 2.5:
                ...
                if circle.status == STARTED:
                    # is it time to stop? (based on global clock, using actual start)
                    if tThisFlipGlobal > circle.tStartRefresh + 2-frameTolerance:
                        # keep track of stop time/frame for later
                ...
                if token.status == NOT_STARTED and tThisFlip >= 0.5-frameTolerance:
                    # keep track of start time/frame for later
                ...
                if token.status == STARTED:
                    # is it time to stop? (based on global clock, using actual start)
                    if tThisFlipGlobal > token.tStartRefresh + 2-frameTolerance:
                        # keep track of stop time/frame for later
