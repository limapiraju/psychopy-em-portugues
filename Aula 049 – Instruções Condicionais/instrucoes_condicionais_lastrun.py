...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='C:\\Users\\limap\\OneDrive\\Área de Trabalho\\Python para Psicólogos\\psychopy-em-portugues\\Aula 049 – Instruções Condicionais\\instrucoes_condicionais_lastrun.py',
...
def run(expInfo, thisExp, win, inputs, globalClock=None, thisSession=None):
    ...
    conditions = ["structural", "sentence", "rhyme"]
    random.shuffle(conditions)
    
    expInfo["ID"] = expInfo["ID"].zfill(3)
    
    participant_code = expInfo["ID"] + conditions[0][:3].upper()
    ...
    # lista de instruções
    instruction_list = ["instruction1.jpg", "instruction2.jpg", "instruction3.jpg", f"instruction4_{conditions[0]}.jpg", "instruction5.jpg"]
    ...
    main_task_msg = visual.TextStim(win=win, name='main_task_msg',
        text='[inserir tarefa principal aqui]',
        font='Times New Roman',
    ...
    # keep track of which components have finished
    main_taskComponents = [main_task_msg, main_task_resp]
    ...
    while continueRoutine:
        ...
        # *main_task_msg* updates
        
        # if main_task_msg is starting this frame...
        if main_task_msg.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
            # keep track of start time/frame for later
            main_task_msg.frameNStart = frameN  # exact frame index
            main_task_msg.tStart = t  # local t and not account for scr refresh
            main_task_msg.tStartRefresh = tThisFlipGlobal  # on global time
            win.timeOnFlip(main_task_msg, 'tStartRefresh')  # time at next scr refresh
            # add timestamp to datafile
            thisExp.timestampOnFlip(win, 'main_task_msg.started')
            # update status
            main_task_msg.status = STARTED
            main_task_msg.setAutoDraw(True)
        
        # if main_task_msg is active this frame...
        if main_task_msg.status == STARTED:
            # update params
            pass
        ...
        if main_task_resp.status == STARTED and not waitOnFlip:
            ...
            if len(_main_task_resp_allKeys):
                main_task_resp.keys = _main_task_resp_allKeys[-1].name  # just the last key pressed
                main_task_resp.rt = _main_task_resp_allKeys[-1].rt
                main_task_resp.duration = _main_task_resp_allKeys[-1].duration
                # a response ends the routine
                continueRoutine = False
